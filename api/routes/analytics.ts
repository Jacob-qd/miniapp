...
import { supabaseAdmin, TABLES, type VisitAnalytics } from '../config/supabase.js';
...
router.get('/overview', authenticateToken, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // 统计页面访问次数
    const pageStats = popularPages?.reduce((acc: Record<string, number>, item) => {
    ...
    const regionStats = regionData?.reduce((acc: Record<string, number>, item) => {
      if (item.user_region) {
 ...
 */
router.get('/devices', authenticateToken, async (req: Request, res: Response): Promise<void> => {
  try {
    const { days = 7 } = req.query;
    const daysAgo = new Date();
    daysAgo.setDate(daysAgo.getDate() - Number(days));
    ...
    const deviceStats = devices?.reduce((acc: Record<string, number>, item) => {
      if (item.device_type) {
 ...
 */
router.get('/content', authenticateToken, async (req: Request, res: Response): Promise<void> => {
  try {
    // 轮播图数量
